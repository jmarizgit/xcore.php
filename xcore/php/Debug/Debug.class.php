<?php
/*
	@type: CLASS
	@author: JOSE MARIZ MELO
	@company: XCHEMA
	@release: 07/18/2010 
	@updated: 05/18/2012
	@description: Debug system, this class is iherited by all classes on the framework
*/
//warning messages stylesheet and 64 images
define("DEBUG_STYLE", '<style type="text/css">div{font-family:Arial,"Verdana";font-size:12px}.alert_success,.alert_error,.alert_help,.alert_warning{padding:10px;margin:3px;padding-left:35px;border:solid 1px;background-repeat:no-repeat;background-position:7px 9px;line-height:20px;z-index:1000}.alert_success{background-image:url(data:image/gif;base64,);border-color:#66b566;color:green;background-color:#e2ffe5}.alert_success a{color:green;text-decoration:underline}.alert_error{background-image:url(data:image/gif;base64,);border-color:#e98383;color:red;background-color:#ffe2e2}.alert_error a{color:red;text-decoration:underline}.alert_help{background-image:url(data:image/gif;base64,);border-color:#cec5a6;color:#bdb59c;background-color:#fffae9}.alert_help a{color:#bdb59c;text-decoration:underline}.alert_warning{background-image:url(data:image/gif;base64,);border-color:#f9dc6a;color:#F9CC6A;background-color:#fffdd8}.alert_warning a{color:#d5c06c;text-decoration:underline}.alert_opaque{-ms-filter: "progid:DXImageTransform.Microsoft.Alpha(Opacity=50)";filter: alpha(opacity=50);-khtml-opacity: 0.5;-moz-opacity: 0.5;opacity: 0.5;}</style>');


abstract class Debug{
	
	
	private $_XDEBUG = 0;
	
	
	public function superHelp(){	
	
		$HELP  = '	
		<br />
		<b>type:</b>		DEBUG "abstract" Class <br />
		<b>author:</b>		Mariz Melo <br />
		<b>released:</b>	10-11-2010 <br />
		<b>description:</b>	<i>"Provide methods to control DEBUG system messages"</i> <br />
		<br />
		METHODS: <b>debugSTART</b>( (int) 1 ) //just set 1 if want to see the PHP messages<br />
		ex: $instance_var->debugSTART(); //only the system messages<br />
		ex: $instance_var->debugSTART(); //initialize only the system messages<br />
		';//end:HELP
		
		
		$this->debugMESSAGE('H', $HELP); //show help message
		return '';
		
	}
	
	
	
	//START THE DEBUG MESSAGES
	public function debugSTART($phperror = 0){
		
		echo DEBUG_STYLE;	//put the styles for the debug systems
		
		//PHP SYSTEM ERROR MESSAGES
		if ($phperror == 1) {
			
			error_reporting(E_ALL);
			ini_set('display_errors', '1');
			
		}		
			
		//START TO SHOW SYSTEM MESSAGES
		$this->_XDEBUG = 1; //set the object variable to 1 allowing the system report messages
		
		
	}//end: method
	
	
	
	
	//SHOW DEBUG MESSAGES: ARGUMENT VALUES: 'E' - error, 'S' - success, or 'H' - help
	public function debugMESSAGE($chType=0, $strMsg=0){
		
		//verify here the debug variable
		if($this->_XDEBUG){
		
			switch($chType){
				case 'H':
					echo "<div class='alert_help xc_debug'><b>HELP:</b> {$strMsg}</div>";
					break;
				case 'S':
					echo "<div class='alert_success xc_debug'><b>SUCCESS:</b> {$strMsg}</div>";	
					break;
				case 'E':
					echo "<div class='alert_error xc_debug'><b>ERROR:</b> {$strMsg}</div>";	
					break;
				case 'W':
					echo "<div class='alert_warning xc_debug'><b>WARNING:</b> {$strMsg}</div>";	
					break;
				
			}//end:switch
		
		}//end:if
		
	}//end:method
	
	
}//class


?>